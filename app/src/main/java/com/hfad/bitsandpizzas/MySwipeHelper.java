...
public abstract class MySwipeHelper extends ItemTouchHelper.SimpleCallback {
  ...
  private GestureDetector.SimpleOnGestureListener gestureListener = new SimpleOnGestureListener(){
    @Override
  ...
  public class MyButton {
    ...
    public void onDraw(Canvas c, RectF rectF, int pos ){
      ...
      if (imageResId == 0) //If just show text
      {
        ...
        c.drawBitmap(bitmap, (float) ((rectF.left +rectF.right)/2.1), (rectF.top+rectF.bottom)/2, p);
        System.out.println((int)(rectF.left +rectF.right)/2);
      }
      clickRegion = rectF;
      this.pos = pos;
    }
  ...
  public abstract void instantiateMyButton(ViewHolder viewHolder, List<MyButton> buffer);
}
